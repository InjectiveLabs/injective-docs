...
* If you don't have existing non-default subaccounts, you can do a [MsgDeposit](./exchange/#MsgDeposit) to a new default subaccountId, which would be done via importing `getSubaccountId` from `sdk-ts` and setting the `subaccountId` field in [MsgDeposit](./exchange/#MsgDeposit) to `getSubaccountId(injectiveAddress, 1)`.
