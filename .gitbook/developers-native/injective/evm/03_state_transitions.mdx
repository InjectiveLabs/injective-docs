...
The options `authante.NewMempoolFeeDecorator()`, `authante.NewTxTimeoutHeightDecorator()` and `authante.NewValidateMemoDecorator(ak)` are the same as for a Cosmos `Tx`. Click [here](https://github.com/cosmos/cosmos-sdk/blob/main/docs/docs/learn/beginner/04-gas-fees.md#antehandler) for more on the `anteHandler`.
