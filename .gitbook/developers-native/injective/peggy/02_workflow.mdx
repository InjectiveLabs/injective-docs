...
* `Signer` which signs new `Validator Set` updates and `Token Batches` with the `Operator`'s Ethereum keys and submits using [messages](04_messages/#Ethereum-Signer-messages).
* `Oracle` which observes Ethereum events and sends them as [claims](04_messages/#Oracle-messages) to Injective.
