...
flowchart LR
    ...
    CALLER -->|"1 · quoteExactInputV1 (read)"| SWAP
    CALLER -->|"2 · swapExactInputV1 (tx)"| SWAP
    SWAP -->|"3 · market allowed?"| PARAMS
    SWAP -->|"4 · execute on default subaccount"| EXCH
    EXCH -->|"5 · fill, or revert below minOut"| SWAP
    SWAP -->|"6 · credit amountOut to recipient"| BANK
    ...
    style EVM fill:transparent,stroke:#4D3DFF,stroke-dasharray:4
    style NATIVE fill:transparent,stroke:#8B94A7,stroke-dasharray:4
```

One transaction covers the whole path: the precompile validates the market against `SwapParams`, places the order on the caller's default subaccount, and either credits the output to the recipient or reverts the entire call. The quote functions follow the same path without changing state.
