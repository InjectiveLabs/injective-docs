...
Component versions verified against Injective Mainnet {/* $VERSION */}. The official SDK releases are identical to the `-rc` versions published during the testnet cycle — if you are already running the `-rc` build, no migration is required.
