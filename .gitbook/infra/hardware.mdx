...
|---|---|---|
| **RAID0** | Stripes data across both drives (e.g. 512 KiB chunks). Best sequential and random throughput. | No redundancy. If either drive fails, the array is lost. |
| **LVM linear** | Concatenates drives into a single logical volume. Simpler setup. | No redundancy. If the drive holding active data fails, data is lost. |

Neither layout provides redundancy, but chain state does not require it. If a drive fails, recover from a [snapshot](https://chain-portal.injective.network/snapshots). RAID0 is preferred for the higher throughput.
