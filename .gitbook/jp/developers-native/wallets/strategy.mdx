...
`@injectivelabs/wallet-strategy`の主な目的は、Injective上で異なるウォレット実装を扱う方法を開発者に提供することです。これらのウォレット実装はすべて同じ`ConcreteStrategy`インターフェースを公開しているため、ユーザーは特定のウォレットの実装を意識することなく、抽象化されたメソッドをそのまま使用できます。
