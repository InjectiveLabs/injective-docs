...
[cosmos sdkのドキュメント](https://docs.cosmos.network/sdk/latest/modules/authz)によると、「認可は特定のMsgサービスメソッドごとに1つずつ付与される必要がある」とされています。したがって、`grantee`が`granter`に代わって認可を持つようにしたい各メッセージ型ごとに、以下のコードスニペットを繰り返す必要があります。
