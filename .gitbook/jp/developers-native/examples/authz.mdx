...
認可のgrantはMsgGrantメッセージを使用して作成されます。すでに(granter, grantee, Authorization)の三つ組に対するgrantが存在する場合、新しいgrantで以前のものが上書きされます。既存のgrantを更新または延長するには、同じ(granter, grantee, Authorization)三つ組で新しいgrantを作成する必要があります。
