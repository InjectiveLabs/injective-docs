...
このドキュメントの目的は、Ledgerを使用してInjective上のトランザクションに署名し、チェーンにブロードキャストする方法を説明することです。Injectiveは鍵にEthereumのECDSA secp256k1曲線を使用する独自のカスタムAccount型を定義しているため、実装はCosmos SDKネイティブチェーンのデフォルトのアプローチとは異なります。
