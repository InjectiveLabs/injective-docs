...
`injective-test-tube`はCosmWasm × Injectiveの統合テストライブラリで、`cw-multi-test`とは異なり、モックではなくチェーンの実際のロジックに対してCosmWasmコントラクトをテストできます。
...
なお、`init_accounts`は同じ初期残高を持つ複数のアカウントを作成する便利な関数です。1つのアカウントだけを作成したい場合は、代わりに`init_account`を使用できます。
