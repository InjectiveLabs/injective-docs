...
基礎知識として役立つ資料：[Gas Metering](https://docs.cosmos.network/sdk/latest/learn/concepts/context-gas-events#gas-metering)、[Auth Module Gas & Fees](https://docs.cosmos.network/sdk/latest/modules/auth/auth#gas-&-fees)
</Callout>

<Callout icon="info" color="#22C55E" iconType="regular">
**exchangeモジュールのガスヒューリスティクス：** exchangeモジュールのメッセージタイプごとの詳細なガスコスト（注文の発注、キャンセル、マーケット操作）については、[Gas Heuristicsリファレンス](/developers-native/injective/exchange/12_gas_heuristics)を参照してください。
</Callout>
...
Cosmos SDKにおけるGasの詳細については、[こちら](https://github.com/cosmos/cosmos-sdk/blob/main/docs/docs/learn/beginner/04-gas-fees.md)をご覧ください。
...
Cosmos SDK [`AnteHandler`](https://docs.cosmos.network/sdk/latest/modules/auth)は、トランザクション実行前に基本的なチェックを行います。これらのチェックは通常、署名検証、トランザクションフィールドの検証、トランザクション手数料の検証・徴収などです。
