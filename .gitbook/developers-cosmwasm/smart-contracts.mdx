---
title: CosmWasm Smart Contracts
---

## What is CosmWasm?

CosmWasm is a novel smart contracting platform built for the Cosmos ecosystem. You can learn more about CosmWasm from [the CosmWasm docs](https://cosmwasm.cosmos.network/), or see the [CosmWasm Book](https://book.cosmwasm.com/index.html) for a guide on creating CosmWasm smart contracts.

## Developing CosmWasm Smart Contracts

New to CosmWasm?
We suggest that you follow this guide: [Your First CosmWasm Smart Contract](./smart-contracts/your-first-smart-contract/)

## Notable Cosmwasm Smart Contracts

| Topic                                                                                      | Description            |
| ------------------------------------------------------------------------------------------ | ---------------------- |
| [Injective Name Service](./smart-contracts/injective-name-service/)                                        | Injective Name Service |
| [Neptune Service](./smart-contracts/neptune-service/)                                                      | Neptune Service        |
| [CW20 to Bank & Market Order in One Transaction](./smart-contracts/cw20-convert-market-order/) | Convert Cw20 Example   |
