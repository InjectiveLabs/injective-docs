...
$$Uptime_{Epoch}$$ is the number of order book snapshots throughout the epoch in which the liquidity provider had a [positive Bid Liquidity Score _**and**_ a positive Ask Liquidity Score](/defi/open-liquidity-program/scoring#liquidity-score) in the market of interest. This means the liquidity provider quoted on both sides of the order book with order sizes greater than or equal to $$MinDepth$$ with spreads less than or equal to $$MaxSpread$$ in the snapshot.
