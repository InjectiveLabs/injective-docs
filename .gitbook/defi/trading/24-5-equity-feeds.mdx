...
description: "How 24/5 pricing for iAssets works on Injective."
---
...
Injective uses SEDA to combine these four session feeds into one oracle price per symbol, updating every ~2 seconds. This unified stream becomes the canonical index price for Injective equity perps.
...
1. **True 24/5 Equity Trading**: Equity perps on Injective can track price action across all four US equity sessions without interruption.
2. **Cleaner Pricing**: No juggling multiple feeds, no abrupt jumps caused by switching sessions—SEDA standardizes everything into one continuous stream.
...
- Volatility varies dramatically by session. Pre-market and after-hours can be more illiquid - expect wider spreads despite continuous pricing.
