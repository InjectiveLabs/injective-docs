...
`feegrant` 模块允许账户（授权者）向其他账户（被授权者）授予费用津贴。这允许被授权者使用授权者的资金支付交易费用。

## 消息

### MsgGrantAllowance

使用 `MsgGrantAllowance` 消息创建费用津贴授权。如果已存在（授权者、被授权者）对的授权，则新授权将覆盖之前的授权。
...
可以使用 MsgRevokeAllowance 消息撤销授权。被授权者将不再能够使用授权者的资金支付交易费用。
