...
此 `tokenfactory` 模块允许任何账户创建名为 `factory/{creator address}/{subdenom}` 的新代币。由于代币按创建者地址命名空间，这使得代币铸造无需许可，因为不需要解决名称冲突。

_注意：如果您希望您的 denom 在 Helix、Hub、Explorer 等产品上可见，重要的是使用下面解释的 `MsgSetDenomMetadata` 消息添加代币元数据信息。_

_注意 #2：建议将您的管理员更改为零地址，以确保安全并防止供应操纵。_
...
特定 denom 的铸造仅允许当前管理员。注意，当前管理员默认为 denom 的创建者。
...
为特定 denom 设置元数据仅允许 denom 的管理员。它允许覆盖银行模块中的 denom 元数据。
...
const msg = MsgSetDenomMetadata.fromJSON({
  ...
  metadata: {
    ...
    name: '', /** 代币名称 */
    symbol: '', /** 代币符号 */
    uri: '' /** 代币 logo，应托管在 IPFS 上，应为小型 webp 图像 */
...
denom 的管理员可以铸造新供应量或销毁现有供应量。建议将管理员更改为零地址，以不允许更改代币的供应量。
