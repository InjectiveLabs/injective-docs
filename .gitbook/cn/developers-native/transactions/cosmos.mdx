...
Injective 上的每笔交易都遵循相同的流程。该流程包括三个步骤：准备、签署和广播交易。让我们分别深入每个步骤并详细解释过程（包括示例），以便我们理解整个交易流程。

## 准备交易

首先，我们需要准备交易以进行签名。

此时您**不能**使用一些在线抽象来根据提供的消息和签名者快速准备交易（例如使用 `@cosmjs/stargate` 包）。原因是这些包不支持 Injective 的 publicKey typeUrl，所以我们必须在客户端进行地址准备。
...
准备好交易后，我们继续签名。从上一步获取 `txRaw` 交易后，使用任何 Cosmos 原生钱包进行签名（例如：Keplr）。
...
您也可以使用我们的 `@injectivelabs/wallet-strategy` 包来获取开箱即用的钱包提供者，这些提供者将为您提供可用于签署交易的抽象方法。请参阅该包的文档，设置和使用非常简单。**这是推荐的方式，因为您可以在 dApp 中使用多个钱包。`WalletStrategy` 提供的不仅仅是签署交易的抽象。**

## 广播交易

签名准备好后，我们需要将交易广播到 Injective 链本身。从第二步获取签名后，我们需要将该签名包含在已签名的交易中并将其广播到链上。
