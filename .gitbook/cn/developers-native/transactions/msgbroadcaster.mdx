...
`MsgBroadcaster` 抽象类是一种在 Injective 上轻松广播交易的方式。使用它，你可以传递一个想要打包到交易中的消息和签名者的地址，交易将被准备、签署和广播。

使用示例可以在我们的 [Helix 演示仓库](https://github.com/InjectiveLabs/injective-helix-demo)中找到。至于可以传递给 `broadcast` 方法的消息，你可以在文档的[核心模块](../examples/)部分找到示例。
...
export interface MsgBroadcasterOptions {
  ...
  gasBufferCoefficient?: number /** 可选 - 作为 gas 缓冲添加到模拟/硬编码的 gas 中，以确保交易包含在区块中 */
}
...
<Callout icon="info" color="#07C1FF" iconType="regular">
  要覆盖 `endpoints` 并使用你的基础设施（这是我们推荐的），请在
  [网络](../../developers/concepts/networks/)页面上阅读更多关于你需要提供的端点以及如何设置它们的信息。
</Callout>
