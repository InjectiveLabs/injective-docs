...
## 使用 Ledger 在 Injective 上签署交易

本文档的目标是解释如何使用 Ledger 在 Injective 上签署交易并将其广播到链上。该实现与 Cosmos SDK 原生链的默认方法不同，因为 Injective 定义了自己的自定义账户类型，使用以太坊的 ECDSA secp256k1 曲线作为密钥。
...
派生路径是一段数据，告诉分层确定性（HD）钱包如何在密钥树中派生特定密钥。派生路径作为标准使用，并作为 BIP32 的一部分与 HD 钱包一起引入。分层确定性钱包是一个术语，用于描述使用种子派生许多公钥和私钥的钱包。
...
Ledger 限制为一个 `coin_type` 只能安装一个应用程序。由于我们必须使用以太坊应用程序在 Injective 上签署交易，我们必须探索可用的选项来获取有效签名。可用选项之一是用于哈希和签署类型化结构数据的 `EIP712` 程序。Ledger 暴露了我们将使用的 `signEIP712HashedMessage`。
...
EIP 712 是用于哈希和签署类型化结构数据的标准。对于每个 EIP712 类型数据，用户传递的每个值（需要签名的）都有一个类型表示，解释该特定值的确切类型。除了用户想要签名的值及其类型（EIP712 typedData 的 `PrimaryType`）外，每个 EIP712 类型数据都应包含一个 `EIP712Domain`，提供有关交易来源的上下文。

## 交易流程

实现本身包括几个步骤，即：

1. 准备交易以使用 Ledger 上的以太坊应用程序签署，
2. 在 Ledger 上准备和签署交易，
3. 准备交易以进行广播，
4. 广播交易。

我们将深入每个步骤并详细说明我们需要采取的操作，以使交易签署并广播到链上。

### 准备交易（用于签署）

如上所述，交易需要使用 Ledger 上的以太坊应用程序签署。这意味着一旦用户到达签署阶段，必须提示用户切换（或打开）Ledger 上的以太坊应用程序。

我们知道每个 Cosmos 交易都由消息组成，这些消息表示用户想要在链上执行的指令。如果我们想从一个地址向另一个地址发送资金，我们将把 `MsgSend` 消息打包到交易中并将其广播到链上。

知道这一点，Injective 团队对这些消息进行了[抽象](https://github.com/InjectiveLabs/injective-ts/blob/master/packages/sdk-ts/src/core/modules/MsgBase.ts)，以简化它们打包到交易中的方式。每个消息都接受一组特定的参数，这些参数是实例化消息所需的。完成后，抽象暴露了几个方便的方法，我们可以根据选择的签署/广播方法使用。例如，消息暴露了 `toDirectSign` 方法，该方法返回消息的类型和 proto 表示，然后可以使用默认的 Cosmos 方法打包交易，使用 privateKey 签署并广播到链上。

对于这个特定实现，重要的是 `toEip712Types` 和 `toEip712` 方法。在消息实例上调用第一个方法会给出 EIP712 类型数据的消息类型，第二个方法给出 EIP712 数据的消息值。当我们结合这两个方法时，我们可以生成有效的 EIP712 类型数据，可以传递给签署过程。

所以，让我们看一个使用这些方法的快速代码片段，以及如何从消息生成 EIP712 typedData：
...
现在我们有了 `eip712TypedData`，我们需要使用 Ledger 签署它。首先，我们需要根据用户在浏览器上的支持获取 Ledger 的传输，并使用 `@ledgerhq/hw-app-eth` 创建一个 Ledger 实例，该实例将使用 Ledger 设备上的以太坊应用程序执行用户的操作（确认交易）。从步骤 1 获取 `eip712TypedData` 后，我们可以使用 `EthereumApp` 上的 `signEIP712HashedMessage` 签署此 typedData 并返回签名。
