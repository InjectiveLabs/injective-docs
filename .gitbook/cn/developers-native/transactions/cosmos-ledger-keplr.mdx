...
本页面将介绍当用户通过 Keplr 钱包使用 Ledger 设备时，Injective 的实现方式。

如前所述，Injective 使用与其他 Cosmos 链不同的派生曲线，这意味着用户必须使用以太坊应用程序（目前）与 Injective 进行交互。

如果您想要一个涵盖所有边缘情况的完整开箱即用解决方案，支持 Injective 上所有支持的钱包，建议您查看 [MsgBroadcaster + WalletStrategy](./msgbroadcaster/#msgbroadcaster-+-wallet-strategy) 抽象。如果您想自己实现，让我们一起看看代码示例。

## 概述

Keplr 暴露了一个 `experimentalSignEIP712CosmosTx_v0` 方法，可用于签署 EIP712 类型数据（通过将 Cosmos StdSignDoc 传递给上述方法，在 Keplr 端自动生成），并允许 EVM 兼容链在通过 Keplr 连接 Ledger 设备时获得正确的签名。
 ...
 * 与 signEthereum(..., EthSignType.EIP712) 的区别在于，此 API 返回用户费用设置更改后的新签名文档以及该签名文档的签名。
 * 将 tx 编码为 EIP-712 格式应在使用此 API 的一侧完成。
 * 与 cosmjs 不兼容。
 * 返回的签名是以太坊中使用的 (r | s | v) 格式。
 * v 应该是 27 或 28，这在以太坊主网中使用，与链无关。
 * @param chainId
 * @param signer
 * @param eip712
 * @param signDoc
...
我们现在需要做的是生成 `eip712` 和 `signDoc`，将它们传递给此函数，Keplr 将要求用户使用其 Ledger 设备上的以太坊应用程序签署交易。

## 示例实现

基于上述概述，现在让我们展示一个完整的示例，说明如何使用 Ledger + Keplr 在 Injective 上签署交易。请注意，以下示例假设您正在使用从 `@injectivelabs/sdk-ts` 包导出的 [Msgs](https://github.com/InjectiveLabs/injective-ts/blob/master/packages/sdk-ts/src/core/modules/msgs.ts#L60) 接口。
...
/**
 * 仅当我们想在 Injective 上使用 Keplr 上的 Ledger 广播交易时使用此方法
 *
 * 注意：Gas 估算不可用
 * @param tx 需要广播的交易
 */
