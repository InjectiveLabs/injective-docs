...
在本文档中，我们将向您展示如何使用私钥在 Injective 上签署交易。
...
准备好交易后，我们继续签名。从上一步获取 `txRaw` 交易后，使用任何 Cosmos 原生钱包签名（例如：Keplr），
...
您可以使用 `@injectivelabs/sdk-ts` 包中的 `MsgBroadcasterWithPk` 类，它将上述大部分逻辑抽象到一个类中。
