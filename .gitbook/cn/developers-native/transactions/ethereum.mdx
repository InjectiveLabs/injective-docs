...
Injective 上的每笔交易都遵循相同的流程。该流程包括三个步骤：准备、签名和广播交易。让我们分别深入每个步骤，详细解释流程（包括示例），以便我们理解整个交易流程。

## 准备交易

首先，我们需要准备交易以进行签名。要使用以太坊原生钱包，我们必须将交易转换为 EIP712 类型数据，并使用钱包签署此类型数据。

使用我们的消息自定义抽象，允许开发者直接从特定消息的 proto 文件获取 EIP712 TypedData。
...
准备好 EIP712 类型数据后，我们继续签名。

```ts
/** 使用您喜欢的方法签署 EIP712 TypedData，以 Metamask 为例 */
...
您也可以使用我们的 `@injectivelabs/wallet-strategy` 包来获取开箱即用的钱包提供程序，它将为您提供可用于签署交易的抽象方法。请参阅该包的文档，设置和使用非常简单。**这是推荐的方式，因为您可以在 dApp 中使用多个钱包。`WalletStrategy` 提供的不仅仅是签署交易的抽象。**

## 广播交易

签名准备好后，我们需要将交易广播到 Injective 链本身。从第二步获得签名后，我们需要将该签名包含在已签名的交易中并将其广播到链上。
