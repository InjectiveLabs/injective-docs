...
Injective Ethereum 跨链桥使 Injective Chain 能够支持无需信任的链上双向代币桥接。在这个系统中，Ethereum 上的 ERC-20 代币持有者可以将其 ERC-20 代币转换为 Injective Chain 上的原生代币，反之亦然。
...
- 下面的代码片段实例化了一个 PeggyContract 实例，可以使用我们提供给合约构造函数的 `web3` 轻松进行 `estimateGas` 和 `sendTransaction`。其实现可以在[这里](https://github.com/InjectiveLabs/injective-ts)找到。显然，这只是一个示例，你可以直接使用 web3 包 + 合约的 ABI 来实例化合约，然后使用某个 web3 提供者处理签名和广播交易的逻辑。
