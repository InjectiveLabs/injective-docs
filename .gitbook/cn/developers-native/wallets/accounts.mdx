...
你可以使用 `@injectivelabs/sdk-ts` 包中的实用函数轻松地在 Injective 地址和以太坊地址之间进行转换：
...
由于 Injective 的派生路径与默认的 Cosmos 路径不同，你需要账户的 `publicKey` 才能将 Cosmos `publicAddress` 转换为 Injective 地址。
