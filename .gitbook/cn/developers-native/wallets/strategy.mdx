...
首先，你需要创建一个 `WalletStrategy` 类的实例，这使你能够开箱即用地使用不同的钱包。你可以使用 `walletStrategy` 实例上的 `setWallet` 方法来切换当前使用的钱包（注意：`setWallet` 是异步的，需要 `await`）。
...
export interface WalletStrategyEvmOptions {
  ...
  evmChainId: EvmChainId; // 如果你使用钱包策略签署 EIP712 类型数据则需要
}
...
_注意：_ 当我们想在以太坊原生钱包上使用 `sendTransaction` 时，除了其他选项（chainId 和 address）外，我们还需要传递一个 gRPC 端点到 sentry 来广播交易。这是必需的，因为从以太坊原生钱包中，我们无法像在 Keplr 或 Leap 上那样访问 `broadcastTx` 方法来使用钱包的抽象广播交易，所以我们必须在客户端直接向链广播。
