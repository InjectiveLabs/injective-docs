...
Injective 支持以太坊和 Cosmos 原生钱包。你可以使用 Metamask、Ledger、Keplr、Leap 等流行钱包在 Injective 上签署交易。

### 钱包策略

支持所有这些钱包的推荐方式是使用我们构建的 [WalletStrategy](./strategy/) 抽象。这种方法将使你的 dApp 用户能够连接和使用不同的钱包进行交互。

将其与 [MsgBroadcaster](../transactions/msgbroadcaster/) 抽象结合使用，可以让你通过一个函数调用来签署交易。这是 Helix、Hub、Explorer 等所有产品中使用的方法，我们强烈建议在你的 dApp 中使用这种方法。
