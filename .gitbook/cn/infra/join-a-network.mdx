...
本指南将引导你完成在本地设置独立网络以及在主网或测试网上运行节点的过程。
...
`$MY_VALIDATOR_ADDRESS` 是保存密钥环中 `my_validator` 密钥地址的变量。Injective 中的代币具有 `{amount}{denom}` 格式：`amount` 是 18 位精度的十进制数，`denom` 是带有其面额键的唯一代币标识符（例如 `inj`）。这里，我们授予 `inj` 代币，因为 `inj` 是 `injectived` 中用于质押的代币标识符。
...
两个文件都有大量注释——请直接参考它们来调整你的节点。

一个需要调整的示例配置是 `app.toml` 中的 `minimum-gas-prices` 字段，它定义了验证者节点愿意接受的处理交易的最低 gas 价格。如果为空，请确保使用某个值编辑该字段，例如 `10inj`，否则节点将在启动时停止。在本教程中，让我们将最低 gas 价格设置为 0：
