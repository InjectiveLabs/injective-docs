...
* **Factory Denom** - 这些 denom 代表 Cosmwasm 上 CW20 代币在 Injective 原生 bank 模块上的表示。它们的格式为 `factory/{CW20_ADAPTER_CONTRACT}/{CW20_CONTRACT_ADDRESS}`，其中 `CW20_ADAPTER_CONTRACT` 是转换 CW20 和原生 Bank 模块的适配器合约地址。

我们在链下维护代币元数据列表，以便更快地访问 [injective-lists](https://github.com/InjectiveLabs/injective-lists/tree/master/tokens) 仓库。
