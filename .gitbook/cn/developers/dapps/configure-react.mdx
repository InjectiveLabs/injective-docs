...
按照 [Vite 文档](https://vitejs.dev/guide/)中的入门指南设置你的应用程序。
...
任何加密相关去中心化应用程序的主要依赖项之一是 `Buffer`。为了确保我们将 `Buffer` 添加到项目中，我们可以将其作为依赖项安装，然后将其导入到 global/window 对象。
...
if (!window.Buffer) {
    window.Buffer = Buffer; // 可选，用于期望 Buffer 为全局的包
}
```
