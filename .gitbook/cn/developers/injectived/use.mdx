...
以下页面解释了通过 `injectived`（连接到 Injective 的命令行界面）可以做什么。你可以使用 `injectived` 与 Injective 区块链交互，包括上传智能合约、查询数据、管理质押活动、处理治理提案等。
...
如果从 Docker 运行，你必须将 home 目录挂载到容器。
...
[运行自己的节点](/cn/infra/join-a-network/)仅适用于高级用户。对于大多数用户，建议连接到公共节点。
...
有关 `injectived` 的更多通用信息，请运行：

```bash
injectived --help
```

有关特定 `injectived` 命令的更多信息，请在命令后附加 `-h` 或 `--help` 标志。例如：
...
要配置更多 `injectived` 选项，请编辑 `~/.injective/config/` 目录中的 `config.toml` 文件。当 keyring-backend 设置为 `file` 时，Keyring 文件位于 `~/.injective/keyring-file` 目录中。也可以将 keyring-backend 设置为 `test` 或 `os`。对于 test，它也将作为文件存储在 `~/.injective/keyring-test` 中，但不受密码保护。
...
* 使用 keyring 的账户签署生成的交易。
* 将签名的交易广播到网络。这是可能的，因为 CLI 连接到公共 Injective 节点的 RPC 端点。

CLI 将所有必要的步骤捆绑到一个简单易用的用户体验中。但是，也可以单独运行所有步骤。
...
这将在控制台中以 JSON 格式输出未签名的交易。我们还可以通过在上述命令后附加 `> unsigned_tx.json` 将未签名的交易保存到文件中（以便在签名者之间更容易传递）。

### 签署预生成的交易

使用 CLI 签署交易需要将未签名的交易保存在文件中。假设未签名的交易在当前目录中名为 `unsigned_tx.json` 的文件中（请参阅上一段了解如何执行此操作）。然后，只需运行以下命令：

```bash
injectived tx sign unsigned_tx.json --from=MY_WALLET
```

此命令将解码未签名的交易并使用 `MY_WALLET` 的密钥以 `SIGN_MODE_DIRECT` 签名，我们已经在 keyring 中设置了该密钥。签名的交易将以 JSON 格式输出到控制台，如上所述，我们可以通过在命令行后附加 `> signed_tx.json` 将其保存到文件中。
...
* `--sign-mode`：你可以使用 `amino-json` 以 `SIGN_MODE_LEGACY_AMINO_JSON` 签署交易，
...
使用多个签名者签名是通过 `tx multi-sign` 命令完成的。此命令假设所有签名者都使用 `SIGN_MODE_LEGACY_AMINO_JSON`。流程类似于 `tx sign` 命令流程，但不是签署未签名的交易文件，而是每个签名者签署前一个签名者签署的文件。`tx multi-sign` 命令将签名附加到现有交易。重要的是，签名者按照交易给出的**相同顺序**签署交易，这可以使用 `GetSigners()` 方法检索。
