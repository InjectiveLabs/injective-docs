...
Injective 문서는 Claude, Cursor 및 기타 MCP 호환 도구와 같은 AI 어시스턴트가 Injective 문서에서 **정보를 검색하고 조회**할 수 있도록 하는 Model Context Protocol(MCP) 서버를 제공합니다.

이것은 문서 검색뿐만 아니라 다른 MCP 서버 및 AI 에이전트와 **함께 사용할 때** 유용합니다.
예를 들어, Cursor와 같은 에이전트 개발 도구에 이 MCP 서버를 연결하여 LLM의 범용 소프트웨어 작성 능력에 **Injective 특화 지식을 추가**하여 Injective 애플리케이션을 바이브 코딩할 수 있습니다.
개발자 도구를 구성하는 방법에 대한 자세한 내용은 아래에 포함되어 있습니다.

## MCP란 무엇인가요?

[MCP](https://modelcontextprotocol.io/introduction)는 AI 어시스턴트가 외부 데이터 소스 및 도구에 연결할 수 있도록 하는 **오픈 표준**입니다.
AI 어시스턴트는 학습 데이터에만 의존하는 대신 MCP를 사용하여 문서에서 직접 실시간 권위 있는 정보에 액세스할 수 있습니다.

## Injective MCP 서버를 사용하는 이유는 무엇인가요?
...
이것은 특히 Injective에서 구축할 때 유용합니다. AI가 특정 사용 사례에 적합한 문서를 찾는 데 도움을 줄 수 있기 때문입니다.
...
<Tabs>
  <Tab title="모든 MCP 클라이언트">
    ...
    서버는 대부분의 최신 MCP 클라이언트에서 지원하는 Streamable HTTP 전송을 사용합니다.
  </Tab>
  <Tab title="Cursor">
...
어시스턴트는 관련 정보를 찾기 위해 자동으로 `SearchInjectiveDocs` 도구를 사용합니다.

**예시 프롬프트:**

- "Injective에서 지갑을 어떻게 만드나요?"
- "CosmWasm 스마트 컨트랙트 배포 방법을 보여주세요"
- "Injective mainnet의 네트워크 endpoint는 무엇인가요?"
- "Token Factory 모듈은 어떻게 사용하나요?"
- "Injective에서 마진 거래가 어떻게 작동하는지 설명해주세요"

AI가 문서를 검색하고 관련 페이지 링크와 함께 답변을 제공합니다.

## MCP 도구 참조

### `SearchInjectiveDocs`

관련 정보를 찾기 위해 Injective 문서 전체를 검색합니다.

**파라미터:**

| 파라미터 | 타입 | 필수 | 설명 |
|-----------|------|----------|-------------|
| `query` | string | Yes | 관련 문서를 찾기 위한 검색 쿼리 |

**반환:**

문서 페이지에 대한 제목과 직접 링크가 포함된 컨텍스트 콘텐츠입니다.
이 도구는 전체 Injective 문서 코퍼스에 걸쳐 시맨틱 검색을 수행하여, 쿼리를 페이지 콘텐츠, 제목 및 코드 예제와 매칭합니다.
결과는 관련성에 따라 순위가 매겨지고, 쿼리에 가장 잘 답변하는 추출된 스니펫과 추가 읽기를 위한 소스 URL과 함께 반환됩니다.
