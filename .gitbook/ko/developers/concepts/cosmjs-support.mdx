...
다시 한번, 권장되는 접근 방식은 Injective의 표준 접근 방식을 사용하는 것이며, [Cosmos transactions](../../developers-native/transactions/cosmos/)에서 자세히 알아볼 수 있습니다.
## Keplr를 사용한 사용법
Keplr와 함께 `@cosmjs` 패키지의 `@injectivelabs` 대안을 사용하는 방법 예시:
```ts
import {
  PrivateKey,
  InjectiveStargate,
...
```
## CLI/Node 환경에서의 사용법
node 또는 CLI 환경에서 `@cosmjs` 패키지의 `@injectivelabs` 대안을 사용하는 방법 예시.
다시 한번, 권장되는 접근 방식은 Injective의 표준 접근 방식을 따르는 [MsgBroadcasterWithPk](../../developers-native/transactions/private-key/#example-with-msgbroadcasterwithpk) 추상화를 사용하는 것입니다.
```ts
import {
  PrivateKey,
  InjectiveStargate,
...
(async () => {
  ...
  if (txResponse.code !== 0) {
    ...
    console.log(
      `브로드캠스트된 트랜잭션 해시: ${JSON.stringify(
