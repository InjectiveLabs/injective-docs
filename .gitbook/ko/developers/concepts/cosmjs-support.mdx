...
다시 한 번 말씀드리지만, 권장되는 접근 방식은 [Cosmos 트랜잭션](../../developers-native/transactions/cosmos/)에서 자세히 알아볼 수 있는 Injective의 표준 접근 방식을 사용하는 것입니다.
## Keplr 사용
...
다시 한 번 말씀드리지만, 권장되는 접근 방식은 Injective의 표준 접근 방식을 따르기 위해 [MsgBroadcasterWithPk](../../developers-native/transactions/private-key/#example-with-msgbroadcasterwithpk) 추상화를 사용하는 것입니다.
```ts
import {
  PrivateKey,
  InjectiveStargate,
