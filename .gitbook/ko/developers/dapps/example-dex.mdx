...
이 짧은 시리즈에서는 Injective에서 DEX를 구축하는 것이 얼마나 쉬운지 보여드리겠습니다. 오픈 소스 [DEX](https://github.com/InjectiveLabs/injective-dex)가 있으며 모든 사람이 참조하고 Injective에서 구축하는 데 사용할 수 있습니다. 처음부터 시작하려는 사람에게 이것이 적합한 곳입니다.
...
- Spot 및 Derivative 마켓과 오더북 가져오기,
- spot 및 derivative 마켓 모두에서 시장가 주문 넣기,
- Injective 주소의 모든 포지션 보기.
...
// Testnet 환경을 위한 사전 정의된 endpoint 가져오기
// (Kubernetes 인프라를 사용하려므로 TestnetK8s 사용)
...
const strategyArgs: WalletStrategyArguments = {}; /** args 정의 */
...
초기 설정이 완료되면 IndexerAPI에서 마켓을 조회(및 스트리밍)하는 방법과 체인에서 직접 사용자 잔액을 조회하는 방법을 살펴보겠습니다.
...
이러한 함수가 있으면 애플리케이션 어디에서나 호출할 수 있습니다(일반적으로 Nuxt의 Pinia, React의 Context providers 등과 같은 중앙 집중식 상태 관리 서비스).

## 트랜잭션

마지막으로 트랜잭션을 만들어 보겠습니다. 이 예제에서는 다음을 수행합니다:

1. 한 주소에서 다른 주소로 자산 보내기,
2. spot 지정가 주문 만들기,
3. derivative 시장가 주문 만들기.
...
export const makeMsgSend = ({
  ...
  const amount = {
    denom,
    amount: toChainFormat(amount, /** denom의 decimals */).toFixed()
  }
...
메시지가 있으면 `msgBroadcaster` 클라이언트를 사용하여 이러한 트랜잭션을 브로드캠스트할 수 있습니다:
