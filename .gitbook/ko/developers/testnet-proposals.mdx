...
`testnet`에서 제안을 제출하고 싶다고 가정해 봅시다. 제안에 대한 투표 기간이 짧기 때문에 제안이 바로 투표 단계로 들어가지 않도록 보증금을 낮추는 것이 좋습니다. 기본적으로 `min_deposit` 값보다 약간 적어야 합니다.
...
injectived tx oracle grant-provider-privilege-proposal YOUR_PROVIDER \
  YOUR_ADDRESS_HERE \
  --title="제안 제목" \
  --description="PROVIDER를 oracle provider로 등록" \
