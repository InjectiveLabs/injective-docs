...
Peggo는 검증자 노드에서 `injectived`와 함께 실행되는 Peggy 브릿지 오케스트레이터입니다. 네 가지 역할을 담당합니다:

- **Oracle** - Ethereum의 입금 및 브릿지 이벤트를 모니터링한 후 Injective 체인에 보고합니다
- **Signer** - 검증자 세트 업데이트와 트랜잭션 배치에 서명합니다
- **Batch Creator** - 효율적인 브릿징을 위해 아웃바운드 토큰 전송을 배치로 그룹화합니다
- **Relayer** - 서명된 검증자 세트와 트랜잭션 배치를 Ethereum Peggy 컨트랙트에 제출합니다

peggo를 실행하지 않는 검증자는 요구되는 서명 기간 내에 배치 확인 또는 검증자 세트 확인을 놓치면 **감금(jail)**됩니다.
...
Cosmos Keyring 설정에 대해 [여기](https://github.com/cosmos/cosmos-sdk/blob/main/docs/docs/user/run-node/00-keyring.md)에서 자세히 읽을 수 있습니다.
...
Cosmos Keyring 설정에 대해 [여기](https://github.com/cosmos/cosmos-sdk/blob/main/docs/docs/user/run-node/00-keyring.md)에서 자세히 알아보세요.
