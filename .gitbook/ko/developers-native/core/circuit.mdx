...
* `CircuitBreakerDecorator` [ante handler](https://github.com/cosmos/cosmos-sdk/blob/main/docs/docs/learn/advanced/00-baseapp.md#antehandler)에서:

```go reference
https://github.com/cosmos/cosmos-sdk/blob/x/circuit/v0.1.0/x/circuit/ante/circuit.go#L27-L41
```

* [message router check](https://github.com/cosmos/cosmos-sdk/blob/main/docs/docs/learn/advanced/00-baseapp.md#msg-service-router)를 통해:
...
:::note
`CircuitBreakerDecorator`는 대부분의 사용 사례에서 작동하지만, [트랜잭션의 내부 메시지를 확인하지 않습니다](https://github.com/cosmos/cosmos-sdk/blob/main/docs/docs/learn/beginner/01-tx-lifecycle.md#antehandler). 따라서 일부 트랜잭션(`x/authz` 트랜잭션 또는 일부 `x/gov` 트랜잭션 등)은 ante handler를 통과할 수 있습니다. **이것은 circuit breaker에 영향을 미치지 않습니다.** message router check가 여전히 트랜잭션을 실패시키기 때문입니다.
