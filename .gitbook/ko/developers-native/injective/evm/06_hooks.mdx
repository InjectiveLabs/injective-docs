...
다음은 `EVMHooks`가 ERC-20 Token을 Cosmos 네이티브 Coin으로 변환하기 위해 네이티브 모듈을 호출하는 컨트랙트를 지원하는 방법을 보여주는 Injective [erc20 모듈](/ko/developers-native/injective/erc20)에서 가져온 예입니다. 위의 단계를 따릅니다.
