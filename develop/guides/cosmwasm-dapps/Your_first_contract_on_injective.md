...
* Understand the basics of a CosmWasm smart contract, learn how to deploy it on Injective, and interact with it using [Injective tools](../../tools/index.md)
...
:::info CosmWasm [State documentation](https://github.com/CosmWasm/docs/blob/archive/dev-academy/develop-smart-contract/01-intro.md#contract-state) :::
...
:::info CosmWasm [InstantiateMsg documentation](https://github.com/CosmWasm/docs/blob/archive/dev-academy/develop-smart-contract/01-intro.md#instantiatemsg) :::
...
:::info CosmWasm [ExecuteMsg documentation](https://github.com/CosmWasm/docs/blob/archive/tutorials/simple-option/develop.md#executemsg-executemsg) :::
...
Only the owner can reset the count to a specific number. See [Logic](Your\_first\_contract\_on\_injective.md#logic) below for the implementation details.
...
:::info CosmWasm [QueryMsg documentation](https://docs.cosmwasm.com/docs/smart-contracts/query) :::
...
See the implementation details in [Logic](Your\_first\_contract\_on\_injective.md#logic-1) below.
...
:::info Read more details on [preparing the Wasm bytecode for production](https://github.com/InjectiveLabs/cw-counter/blob/59b9fed82864103eb704a58d20ddb4bf94c69787/Developing.md#preparing-the-wasm-bytecode-for-production) :::
...
:::info [`injectived`](../../tools/injectived/02\_using.md) is the command-line interface and daemon that connects to Injective and enables you to interact with the Injective blockchain. :::

A Docker image has been prepared to make this tutorial easier. Alternatively, you can follow the [installation instructions](../../../docs/develop/tools/injectived/install/) for `injectived` and run it locally.

:::tip If you install `injectived` from the binary, ignore the docker commands. On the [public endpoints section](https://docs.injective.network/develop/public-endpoints) you can find the right --node info to interact with Mainnet and Testnet. :::
...
:::info Fund your recently generated test address with the [Injective test faucet](https://faucet.injective.network/). :::
...
:::tip There are different ways to find the code that you just stored:
...
:::info Reminder On CosmWasm, the upload of a contract's code and the instantiation of a contract are regarded as separate events. :::
...
:::info You can find the contract address and metadata by:
...
As we know from [earlier](Your\_first\_contract\_on\_injective.md#querymsg), the only QueryMsg we have is `get_count`.
...
We see that `count` is 99, as set when we [instantiated the contract](Your\_first\_contract\_on\_injective.md#instantiate-the-contract).

:::note If you query the same contract, you may receive a different response as others may have interacted with the contract and incremented or reset the count. :::
...
If we [query](Your\_first\_contract\_on\_injective.md#querying-the-contract) the contract for the count, we see:
...
Now, if we [query](Your\_first\_contract\_on\_injective.md#querying-the-contract) the contract again, we see the count has been reset to the provided value:
...
Congratulations! You've created and interacted with your first Injective smart contract and now know how to get started with CosmWasm development on the Injective Chain. Continue to [Creating a Frontend for Your Contract](Creating\_a\_frontend\_for\_your\_contract.md) for a guide on creating a web UI.
